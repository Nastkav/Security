...
//Додам тут трохи коментарів для звіту
app.use((req, res, next) => {
    let fulltoken = req.get(SESSION_KEY);  // я перевіряю, чи є токен
    if (fulltoken) {
        let token = JSON.parse(fulltoken);
        const expiresDate = new Date(token.expiresDate);
        expiresDate.setHours(expiresDate.getHours() - 1); //ось мінусую час на одну годину  
        ...
        if (expiresDate < currentDate) { // тут вже перевіряю чи не випарується токен
